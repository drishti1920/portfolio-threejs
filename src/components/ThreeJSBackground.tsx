...
function FloatingSphere({ position, size, color, speed = 1, offset = 0 }: 
  ...
  useFrame(({ clock }) => {
    if (ref.current) {
      ref.current.position.y = position[1] + Math.sin((clock.getElapsedTime() + offset) * speed) * 0.5;
...
function Scene() {
  ...
  return (
    <>
      ...
      <ResponsiveCamera />
      <group ref={group}>
        <FloatingSphere position={[-4, 0, -2]} size={1.2} color="#626F47" speed={0.6} offset={0} />
        <FloatingSphere position={[4, 1, 2]} size={0.8} color="#d4a373" speed={0.8} offset={2} />
        <FloatingSphere position={[0, -2, 0]} size={1.5} color="#d4a373" speed={0.4} offset={4} />
        <FloatingSphere position={[2, 3, -3]} size={0.7} color="#4a543a" speed={1} offset={6} />
      </group>
    </>
  );
}
